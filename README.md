...
Just A Rather Very Intelligent System — a single-user FastAPI backend plus frontend that provides a Jarvis-style assistant with:

- Groq LLM for chat (general and realtime modes)
- Local vector store (FAISS + sentence-transformers) for personal knowledge
- Tavily web search for up-to-date answers
- Streaming responses with optional inline text-to-speech (edge-tts)
...
3. **Configure environment**
   - Copy `.env` template if you have one, or create `.env` in the project root.
   - Required:
     - `GROQ_API_KEY=...` (you can also set `GROQ_API_KEY_2`, `GROQ_API_KEY_3`, … for fallback)
   - Optional but recommended:
     - `TAVILY_API_KEY=...` (for realtime web search)
     - `ASSISTANT_NAME=Jarvis` (display name)
     - `JARVIS_USER_TITLE=...` (how the assistant addresses you)
     - `JARVIS_OWNER_NAME=...` (your name, for personalization)

4. **Run the server**
   - `python run.py`
   - The API will start on `http://localhost:8000`.
   - If the `frontend/` folder is present, open `http://localhost:8000/app/` in your browser.

## Project layout (backend)

- `run.py` — small wrapper to start Uvicorn with reload.
- `config.py` — central configuration (paths, models, system prompt, env loading).
- `app/main.py` — FastAPI app, lifespan startup, middleware, routes.
- `app/services/`:
  - `vector_store.py` — builds and queries FAISS index.
  - `groq_service.py` — general chat via Groq, with multi-key fallback.
  - `realtime_service.py` — Groq + Tavily realtime mode.
  - `brain_service.py` — Groq-based classifier (general vs realtime for Jarvis mode).
  - `chat_service.py` — session management, persistence, streaming orchestration.
- `app/utils/` — small helpers (time info, retries, key rotation).
- `database/`:
  - `learning_data/` — your `.txt` knowledge files.
  - `chats_data/` — JSON chat histories.
  - `vector_store/` — persisted FAISS index.

## Usage notes

- Add `.txt` files to `database/learning_data/` with information you want Jarvis to know, then restart the server so the vector store rebuilds.
- Use the Jarvis unified streaming route (`/chat/jarvis/stream`) from the frontend to automatically pick between general and realtime modes.
- For best stability, keep `.env` keys valid and avoid editing files inside `.venv`; only change code under the project root.
