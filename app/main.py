...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # --- Step 3b: Brain Service (Groq query classifier for Jarvis mode) ---
...
def _stream_generator(session_id: str, chunk_iter, is_realtime: bool, tts_enabled: bool = False):
    ...
    try:
        # ── Main loop: iterate over LLM text chunks (or search_results, activity in Jarvis) ──
        for chunk in chunk_iter:
            # Jarvis mode yields activity events for the left-side panel.
            if isinstance(chunk, dict) and "_activity" in chunk:
                yield f"data: {json.dumps({'activity': chunk['_activity']})}\n\n"
                continue
            # Realtime/Jarvis may yield search_results for the right-side widget.
...
async def chat_jarvis_stream(request: ChatRequest):
    """
    Unified Jarvis route — brain classifies each query, then routes to general
    or realtime. Uses the SAME session as /chat and /chat/realtime.

    Flow:
      1. Brain (Groq) classifies: general (no web search) or realtime (web search).
      2. Routes to groq_service or realtime_service accordingly.
      3. Streams SSE with activity events (for left panel) and search_results (for realtime).

    Same session as /chat and /chat/realtime. Returns chunks as SSE.
    """
    if not chat_service:
        raise HTTPException(status_code=503, detail="Service not initialized")
